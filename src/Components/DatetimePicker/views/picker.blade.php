<x-dynamic-component :component="WireUi::component('text-field')" x-ref="container" :config="$config" :attributes="$wrapper" x-data="wireui_date_picker"
    :x-props="WireUi::toJs([
        'config' => [
    ...
    x-on:click="positionable.openIfClosed()" x-on:keydown.enter.stop.prevent="positionable.openIfClosed()"
    x-on:keydown.space.stop.prevent="positionable.openIfClosed()"
    x-on:keydown.arrow-down.stop.prevent="positionable.openIfClosed()" tabindex="0">
    <div class="hidden" hidden>
        <x-wireui-wrapper::hidden :id="$id" :name="$name" :value="$value" x-ref="rawInput"
    ...
    @if ($multiple)
        <div class="flex items-center w-full gap-1 overflow-x-auto cursor-pointer hide-scrollbar overscroll-x-contain"
            ...
            <template x-for="(date, index) in selectedDatesDisplay" wire:key="date">
                <button
                    ...
                    "
                    type="button" title="{{ __('wireui::messages.labels.remove') }}"
    ...
    <x-wireui-wrapper::element readonly autocomplete="off" :attributes="$input->except(['readonly', 'disabled', 'required'])" class="cursor-pointer"
        x-bind:value="display" x-show="selectedDates.length === 0" />
        ...
        <x-slot:append class="flex items-center">
            @if ($clearable)
                <x-dynamic-component :component="WireUi::component('icon')"
                    ...
                    name="x-mark" x-show="entangleable.isNotEmpty()" x-on:click.stop.prevent="clear" x-cloak />
            @endif

            <x-dynamic-component :component="WireUi::component('button')" class="h-full" :color="$color ?? 'primary'" :rounded="data_get($roundedClasses, 'append', '')"
                :disabled="$disabled" x-on:keydown.arrow-down.prevent="focusable.walk.to('down')" use-validation-colors
                flat>
                <x-dynamic-component :component="WireUi::component('icon')" :name="$rightIcon" @class([
    ...
    <x-slot:after>
        <x-dynamic-component :component="WireUi::component('popover')" :margin="(bool) $label" class="overflow-hidden sm:w-72"
            root-class="justify-end !sm:w-72 sm:w-72! ms-auto sm:w-full" x-ref="optionsContainer" tabindex="-1"
            ...
            <header class="p-2.5"
                ...
                <div x-show="tab !== 'time-picker'" class="flex items-center justify-between">
                    <div class="flex items-center w-full gap-x-2 text-secondary-600 dark:text-secondary-500">
                        <button
                            ...
                            x-on:click="toggleTab('years-picker')" type="button">
                            <span x-text="calendar.year"></span>

                            <x-dynamic-component :component="WireUi::component('icon')"
                                class="transition-all duration-200 ease-in-out size-3"
                                x-bind:class="{ 'rotate-180': tab === 'years-picker' }" name="chevron-down" gray flat />
                        </button>

                        <button
                            class="cursor-pointer flex items-center gap-x-2 focus:outline-none focus:outline-hidden focus:underline"
                            x-on:click="toggleTab('months-picker')" type="button">
                            <span x-text="$props.calendar.monthNames[calendar.month]"></span>

                            <x-dynamic-component :component="WireUi::component('icon')"
                                class="transition-all duration-200 ease-in-out size-3"
                                x-bind:class="{ 'rotate-180': tab === 'months-picker' }" name="chevron-down" gray
                                flat />
                        </button>
                    </div>

                    <div class="flex items-center">
                        <x-dynamic-component :component="WireUi::component('mini-button')" class="shrink-0" x-on:click="previous"
                            icon="chevron-left" gray flat rounded="lg" />

                        <x-dynamic-component :component="WireUi::component('mini-button')" class="shrink-0" x-on:click="goToday" gray flat rounded>
                            <div class="rounded-full size-2 bg-slate-600"></div>
                        </x-dynamic-component>

                        <x-dynamic-component :component="WireUi::component('mini-button')" class="shrink-0" x-on:click="next" icon="chevron-right"
                ...
                <div x-show="tab === 'time-picker'" class="flex items-center justify-between">
                    ...
                    <x-dynamic-component :component="WireUi::component('mini-button')" icon="calendar-days" flat gray rounded
            ...
            <div class="p-2.5" :class="{ 'px-0': tab === 'time-picker' }">
                    ...
                    <div class="grid grid-cols-3 gap-2">
                        <template x-for="(name, index) in $props.calendar.monthNames" :key="`month.${name}`">
                            <button
                                ...
                                :class="{
                                    'text-white bg-primary-500 font-semibold focus:ring-primary-600': index === calendar
                                        .month,
                                    'hover:bg-primary-100 hover:text-primary-900 hover:font-medium': index !== calendar
                                        .month,
                                ...
                                :disabled="$props.calendar.disabled.months.includes(index)" x-text="name"></button>
                        </template>
                    </div>
                </template>
                    ...
                    <div class="grid grid-cols-3 gap-2">
                        <template x-for="year in calendar.years" :key="`month.${year.number}`">
                            <button
                                ...
                                :disabled="year.isDisabled" x-on:click="selectYear(year.number)"
                                x-text="year.number"></button>
                        </template>
                    </div>
                </template>
                ...
                <div x-show="tab === 'calendar'">
                    <div class="grid grid-cols-7 gap-1">
                        ...
                        <template x-for="day in calendar.dates" :key="day.date">
                            <button @class([
                                ...
                                :class="{
                                    ...
                                    'focus:ring-2 focus:ring-primary-600 focus:ring-inset': day.isSelected && !day
                                        .isDisabled,
                                    ...
                                    'focus:ring-[1.5px] focus:ring-primary-500 focus:ring-inset': !day.isSelected && !
                                        day.isDisabled,
                                ...
                                :disabled="day.isDisabled" x-on:click="selectDay(day)" type="button">
                                <span x-text="day.number"></span>

                                <div x-show="day.isToday" class="absolute rounded-full size-1 bottom-1"
                                    :class="{
                ...
                <template x-if="tab === 'time-picker'">
                    <x-dynamic-component :component="WireUi::component('time-selector')" class="!mt-0 mt-0!" x-model="time" :military-time="$timeFormat == '24'"
                        :without-seconds="$withoutTimeSeconds" borderless shadowless />
                </template>
            </div>
            ...
            @else
                <footer class="flex items-center justify-end w-full gap-2 p-2 rounded-b-xl bg-slate-100"
                    x-show="shouldShowFooter">
                    <x-dynamic-component :component="WireUi::component('button')" x-on:click="cancel" flat gray sm>
                        <span class="text-gray-400 group-hover:text-gray-600 group-focus:text-gray-600">
                            {{ trans('wireui::messages.date_picker.cancel') }}
                        </span>
                    </x-dynamic-component>

                    <x-dynamic-component :component="WireUi::component('button')" :label="trans('wireui::messages.date_picker.apply')" x-on:click="positionable.close()"
                        primary sm />
                </footer>
            @endif
        </x-dynamic-component>
