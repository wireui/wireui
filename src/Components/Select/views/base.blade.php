<x-dynamic-component :component="WireUi::component('text-field')" x-ref="container" :config="$config" x-data="wireui_select" :attributes="$wrapper->class([
    ...
    x-on:click="openIfClosed" x-on:keydown.enter.stop.prevent="openIfClosed" x-on:keydown.space.stop.prevent="openIfClosed"
    ...
    <button type="button" class="cursor-pointer flex items-center w-full truncate border-0 outline-0" tabindex="-1">
        <span
            ...
            x-show="isEmpty()" x-text="getPlaceholder"></span>
    ...
    <x-slot:after>
        <x-dynamic-component :component="WireUi::component('popover')" :margin="(bool) $label" class="w-full overflow-hidden select-none max-h-80"
            ...
            <div class="px-2 my-2" wire:key="search.options.{{ $name }}"
                ...
                <x-dynamic-component :component="WireUi::component('input')" class="bg-slate-100 dark:bg-transparent" x-ref="search"
                    x-model.debounce.500ms="search" shadowless right-icon="magnifying-glass" :placeholder="trans('wireui::messages.search_here')"
                    type="search" />
            </div>

            <div class="overflow-y-auto select-none max-h-60 snap-y snap-proximity overscroll-contain soft-scrollbar"
                ...
                @unless ($hideEmptyMessage)
                    <div class="px-3 py-12 text-center cursor-pointer sm:py-2 sm:px-3 sm:text-start text-secondary-500"
