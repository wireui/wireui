<div x-data="wireui_time_selector"
    {{ $attributes->class([
    ...
    <input x-ref="input" type="hidden" x-model.fill="timeInput" {{ $attributes->whereDoesntStartWith('wire:model') }} />
    ...
    <ul class="w-full" x-ref="seconds" x-show="config.seconds" wire:ignore></ul>

    <ul class="flex flex-col justify-center w-full h-full" style="top: 14px" x-ref="period" x-show="!config.military"
        wire:ignore></ul>
</div>
