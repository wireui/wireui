<div @attributes([
    ...
    <label
        {{ $attributes->whereStartsWith(['x-ref', 'x-on:', 'x-bind:', 'tabindex'])->merge(['for' => $id])->class([
                ...
                $padding => $padding,
                'ps-3' => !$padding && !isset($prepend),
                'pe-3' => !$padding && !isset($append),
        ...
        @if (!isset($prepend) && ($prefix || $icon))
            <div name="form.wrapper.container.prefix"
                ...
                @if ($icon)
                    <x-dynamic-component :component="WireUi::component('icon')" :name="$icon" class="w-4.5 h-4.5 text-current" />
                @elseif($prefix)
                    {{ $prefix }}
                @endif
            </div>
        @elseif (isset($prepend))
            <div name="form.wrapper.container.prepend"
                {{ $prepend->attributes->class(['group/prepend wrapper-prepend-slot', 'flex h-full py-0.5 ps-0.5']) }}>
                {{ $prepend }}
            </div>
        @endif
        ...
        @if (!isset($append) && ($rightIcon || $suffix || $withErrorIcon))
            <div name="form.wrapper.container.suffix"
                ...
                @if ($rightIcon)
                    <x-dynamic-component :component="WireUi::component('icon')" :name="$rightIcon" class="w-4.5 h-4.5" />
                @elseif($suffix)
                    {{ $suffix }}
                @elseif($withErrorIcon)
                    <x-dynamic-component :component="WireUi::component('icon')" name="exclamation-circle"
                        class="hidden invalidated:block w-4.5 h-4.5" />
                @endif
            </div>
        @elseif(isset($append))
            <div name="form.wrapper.container.append"
                {{ $append->attributes->class(['group/append shrink-0 wrapper-append-slot', 'flex h-full py-0.5 pe-0.5']) }}>
                {{ $append }}
            </div>
        @endif
    </label>
