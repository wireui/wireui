...
class SliderInput extends FormComponent
{
    ...
    {
        return $this->range ? 'wireui::components.inputs.slider-range' : 'wireui::components.inputs.slider';
    }
    ...
    public function formatDataSliderRange(ComponentAttributeBag $attributes, string $type)
    {
        ...
        if (is_string($model->directive)) {
            $copy->offsetSet($model->directive, "{$model->value}_{$type}");
        }

        if ($copy->has('name')) {
            $copy->offsetSet('name', "{$copy->get('name')}_{$type}");
        }
